...
"""
Training script without DeepSpeed dependencies.
    ...
    if getattr(trainer.args, "tune_mm_mlp_adapter", False):
        ...
        for k, v in trainer.model.named_parameters():
            if any(key_match in k for key_match in keys_to_match):
                weight_to_save[k] = v.detach().cpu().clone()
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in self.list_data_dict[i]:
            ...
            data_dict['image'] = torch.zeros(1,3, 384, 384)
            data_dict['h_block'] = 1
            data_dict['w_block'] = 1
            data_dict['mode'] = 'text'
        return data_dict
...
class SimpleLLaVATrainer(Trainer):
    """
    Simplified trainer without DeepSpeed dependencies.
    Uses native PyTorch DDP for distributed training.
    """
    ...
    def _get_train_sampler(self,dataset=None) -> Optional[torch.utils.data.Sampler]:
        ...
        # Use standard sampler - group_by_modality_length requires custom sampler
        # that we've simplified out
        return super()._get_train_sampler()

    def create_optimizer(self):
        """
        Setup the optimizer with optional separate learning rate for mm_projector.
        """
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        if getattr(self.args, 'tune_mm_mlp_adapter', False):
            ...
            super(SimpleLLaVATrainer, self)._save_checkpoint(model, trial, metrics)

    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        if getattr(self.args, 'tune_mm_mlp_adapter', False):
            pass
        else:
            super(SimpleLLaVATrainer, self)._save(output_dir, state_dict)
